...
export default function Swap() {
  ...
  const [fromToken, setFromToken] = useState(
    tokenList.filter((item) => item.name == "MATIC")[0]
  );
  const [toToken, setToToken] = useState(
    tokenList.filter((item) => item.symbol == "USDT")[0]
  );
  ...
  const { data, sendTransaction, status } = useSendTransaction({
    from: address,
  ...
  const notific = () =>
    (toastId.current = toast("Transaction pending...", {
      position: "top-center",
      autoClose: false,
    }));
  ...
  useEffect(() => {
    if (isLoading) notific();
    if (isSuccess) update();
  }, [isLoading, isSuccess]);

  const notify = () =>
    toast.info("Please select different tokens", {
      position: "top-right",
      autoClose: 5000,
  ...
  useEffect(() => {
    ...
    let getTokenRates = setTimeout(async () => {
      ...
      try {
        setToAmount(
          await getRates(fromToken.address, toToken.address, weiAmount)
        );
      } catch (e) {
        console.log(e);
        notifyError();
  ...
  async function getUserBalances() {
    ...
    try {
      ...
      balances.map(
        (item) =>
          (finalBalances[item.token_address] = {
  ...
  return (
    <>
      <main>
        <div className="swap-container main-container">
          ...
          <div className="input-containers">
            ...
            />
            <button
              className="switch-currencies-btn"
            ...
            disabled={
              ...
              (fromTokenData.data &&
                parseFloat(fromAmount) >
                  parseFloat(fromTokenData.data?.formatted)) ||
              isLoading ||
              isTxBuilding
            }
          >
            {(!fromAmount && "Enter A Mount") ||
      ...
      {isTxBuilding && (
        <div className="loading-modal">Building Transaction...</div>
      )}
      {status === "loading" && (
        <div className="loading-modal">Confirm in wallet...</div>
      )}
    </>
  );
}
